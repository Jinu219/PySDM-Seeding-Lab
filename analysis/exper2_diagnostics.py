...
EXPER2_BUILD_ID = "exper2-diagnostics-20260713"

ACTIVATED_RADIUS_THRESHOLD_M = 0.5e-6
RAIN_RADIUS_THRESHOLD_M = 25.0e-6


EXPER2_VARIABLE_GROUPS: Dict[str, List[str]] = {
...
EXPER2_PREFERRED_ORDER: List[str] = [
    "water_vapour_mixing_ratio",
    "supersaturation_percent",
    "relative_humidity_percent",
    "temperature_K",
...
def _infer_temperature(df: pd.DataFrame, config: Dict[str, Any] | None) -> pd.Series:
    ...
    # Not a full thermodynamic closure. This gives a useful placeholder-like
    # temperature response when adapters do not explicitly output temperature.
...
def _infer_water_vapour(df: pd.DataFrame, config: Dict[str, Any] | None) -> pd.Series:
    ...
    # Conservative diagnostic proxy: consumed vapour follows liquid growth.
    # The absolute value should be interpreted only if the adapter provides
    # a real water-vapour product.
    return qv0 - liquid


def add_exper2_diagnostics(
    df: pd.DataFrame,
    config: Dict[str, Any] | None = None,
) -> pd.DataFrame:
    """
    Add Exper2/Follow-up style diagnostic columns.

    This function is intentionally conservative:
    - It preserves all adapter-native products.
    - It fills missing Exper2 columns with clear proxy diagnostics where possible.
    - It leaves rain-effective-radius as NaN when no rain-size water exists.
    """
    out = df.copy()
...
def diagnostic_health(df: pd.DataFrame) -> Dict[str, Any]:
    """Return finite-fraction health diagnostics for Exper2 columns."""
    rows: Dict[str, Any] = {}
    for column in EXPER2_PREFERRED_ORDER:
...
def available_exper2_groups(df_columns: List[str]) -> Dict[str, List[str]]:
    """Return Exper2 groups limited to available columns."""
    available = set(df_columns)
    return {
        group: [column for column in columns if column in available]
        for group, columns in EXPER2_VARIABLE_GROUPS.items()
